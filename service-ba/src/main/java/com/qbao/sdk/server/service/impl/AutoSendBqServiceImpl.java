...
import com.qbao.sdk.server.shangsu.PersonTraceResp;
import com.qbao.sdk.server.shangsu.PersonTradeReqInfo;
	...
	* @throws
	...
	private void doSendBq(BqAutoSendEntity entity, StoreAwardConfEntity awardConf) {
		...
		if (num.compareTo(new BigDecimal(0)) == 1) {
            ...
            bizDesc.append("<").append(entity.getAppName()).append(">消费").append(PriceUtil.formatDecimal(PriceUtil.convert2YuanD(costQbb).doubleValue())).append("元赠送").append(num.intValue()).append("钱宝券");
			entity.setBizDesc(bizDesc.toString());

			result= rechargeFanquan(entity);
     ...
     * 会员充值返券（P090301)
     */
    private boolean rechargeFanquan(BqAutoSendEntity entity) {
...
//            log.info("查询用户{}昨日会员等级返回信息:{}", payUserId, responseBody);
            ...
            log.info("获取{}昨日会员等级信息以异常:{}", payUserId,e);
        }
